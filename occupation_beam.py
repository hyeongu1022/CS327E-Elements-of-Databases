...
class FormatSocCode(beam.DoFn):
  def process(self, element):  
    ...
    # if the element is not NULL
    if soc_code != None and soc_code != "N/A" and len(soc_code) <= 10 and len(soc_code) != 7 and "-" in soc_code:  # some are letters like "Computer System Analyst"
...
def run():
     ...
     out_pcoll_no_dup | 'Log no_dup_output' >> WriteToText('output_occ_no_dup.txt')
    
     out_pcoll_fix_date = out_pcoll_no_dup | 'Format Date' >> beam.ParDo(FormatDate())
        
     out_pcoll_fix_date | 'Log fix_date_output' >> WriteToText('output_occ_fix_date.txt')
